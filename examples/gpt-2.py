...
class Transformer(nn.Module):
	...
	self.layers.append(self.tokens_embeddings)
        self.position_embeddings = nn.Embedding(num_max_positions, embed_dim)
        self.layers.append(self.position_embeddings)
        ...
        self.laryer.append(self.feed_forwards)
        self.laryer.append(self.layer_norms_1)
        self.laryer.append(self.layer_norms_2)
	...
	'''
        ...
        for i, layer in enumerate(self.layers):
           if i==0:
               tokens_embeddings = layer[i]
	       h = self.tokens_embeddings(x)
           if i==1:
               position_embeddings = layer[i]
	       h = h + position_embeddings(positions).expand_as(h)
           if i==2:
		dropout = layer[i]  
                h = dropout(h)  
		attn_mask = torch.full((len(x), len(x)), -float('Inf'), device=h.device, dtype=h.dtype)
                attn_mask = torch.triu(attn_mask, diagonal=1)
           if i==3:
		attentions = self.layers[3]
		feed_forward = self.layers[4]
		norm1 = self.layers[5]
		norm2 = self.layers[6]
		for layer_norm_1, attention, layer_norm_2, feed_forward in zip(layer_norms_1, attentions,
                                                                       layer_norms_2, feed_forwards):
           		 h = layer_norm_1(h)
           		 x, _ = attention(h, h, h, attn_mask=attn_mask, need_weights=False)
           		 x = dropout(x)
           		 h = x + h
